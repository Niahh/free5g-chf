...
// newNrfTestConsumer builds a Consumer over a private CHF context pointed at the
// test NRF and intercepts the openapi cleartext HTTP/2 client with gock.
func newNrfTestConsumer(t *testing.T) *Consumer {
	t.Helper()

	consumer := newTestConsumer(t, &chf_context.CHFContext{
		NfId:   testNfId,
		NrfUri: testNrfUri,
	})
...
func TestSendSearchNFInstances(t *testing.T) {
	...
	gock.New(testNrfUri).
		...
		MatchParam("requester-nf-type", "CHF").
		Reply(http.StatusOK).
	...
	consumer := newTestConsumer(t, &chf_context.CHFContext{NrfUri: testNrfUri})
	targetNFType := models.Nrf_NFMgmt_NFType_UDR
	requesterNFType := models.Nrf_NFMgmt_NFType_CHF
...
func TestSendUpdateNFInstanceWithoutNrfUri(t *testing.T) {
	consumer := newTestConsumer(t, &chf_context.CHFContext{NfId: testNfId})
...
func TestRegisterNFInstance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The CHF keeps the instance ID it chose, whatever the NRF echoes back.
